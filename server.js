...
.then(async () => {
  ...
  // Recreate indexes safely — KEEP sparse: true
  try {
    ...
    await User.collection.createIndex({ linkCode: 1 }, { unique: true, sparse: true }); // ✅ Allows field to be missing
    ...
    // ✅ AUTO-FIX LEGACY USERS WITH linkCode: null
    console.log('🛠️  Fixing legacy users with null linkCode...');

    const nullUsers = await User.find({ linkCode: null });
    for (const user of nullUsers) {
      console.log(`🔧 Fixing user: ${user.username || user.email}`);
      user.linkCode = undefined; // Trigger pre-save to generate
      await user.save();
      console.log(`✅ Fixed → New linkCode: ${user.linkCode}`);
    }

    if (nullUsers.length > 0) {
      console.log(`🎉 Fixed ${nullUsers.length} legacy users`);
    } else {
      console.log('✅ No legacy null linkCodes found');
    }

  } catch (err) {
    console.error('❌ Index or migration error:', err);
  }
})
.catch(err => {
...
app.post('/signup', async (req, res) => {
  ...
  try {
    ...
    // ✅ linkCode will be auto-generated by pre-save hook in User model
    const user = await User.create({
      username: cleanUsername,
      email: cleanEmail,
      passwordHash
      // ← linkCode auto-generated by schema pre-save
    });
...
// ✅ /link route — regenerates code if missing
app.get('/link', requireAuth, checkBan, async (req, res) => {
  ...
  // ✅ If no linkCode, generate one
  if (!user.linkCode) {
    user.linkCode = await User.schema.methods.generateUniqueLinkCode.call(user); // Or use standalone fn
    // If you don't have method, use:
    ...
    await user.save();
    console.log(`🆕 Generated new linkCode for ${user.username}: ${user.linkCode}`);
  }

  res.render('link', {
    username: user.username,
    linkCode: user.linkCode,
    inviteUrl: 'https://discord.gg/MmDs5ees4S' // ✅ Trimmed
  });
});
